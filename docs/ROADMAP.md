...
| 5   | **`apps/api` skeleton + WorkOS**            | NestJS app; WorkOS SSO + SCIM/Directory Sync; request-scoped tenant/engagement context interceptor (resolves auth → `withTenant` / `withEngagement`); OpenAPI; health. The seam every later feature hangs off.                                                                                                                               | M   | #4                          |
...
| 10  | **`@fde/authz` (SpiceDB) — platform roles** | SpiceDB schema + typed check/write wrappers; platform-role checks in the read path. Source-ACL mirroring is deferred to M5 (Slack).                                                                                                                                                                                                          | M   | #5                          |
