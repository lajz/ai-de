...
export function createExtractionActivities(deps: ExtractionActivitiesDeps) {
  async function runExtractionActivity(input: RunExtractionInput): Promise<RunExtractionResult> {
    ...
    const outcome = await withEngagementActivity(
      deps.db,
      deps.keyProvider,
      ...
      async (ctx) => {
        ...
        await ctx.tx
          .delete(embeddings)
          .where(and(eq(embeddings.engagementId, engagementId), eq(embeddings.sourceId, sourceId)));
        ...
        for (const chunk of chunks) {
          ...
          const { value: extracted, usage: u } = await deps.router.extract(extractionResultSchema, {
            prompt: { name: EXTRACTION_PROMPT_NAME, version: promptVersion },
            jsonSchema: extractionJsonSchema,
            schemaName: 'extraction_result',
            tier: 'bulk',
            messages: wrapTranscript(chunk.text),
          });
          usage.calls += 1;
